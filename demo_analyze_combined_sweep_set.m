...
for k = 1:N 
    ...
    if (check_required_dir (each_bisweep_dir, each_bisweep_trial))
        
        figure (k); clf;
        % show_bidirectional_sweep (each_bisweep_dir, VA_ETDRS, info)

        descend_datafile = fullfile (each_bisweep_dir, each_bisweep_trial{1}, 'result/signal.csv');
        ascend_datafile  = fullfile (each_bisweep_dir, each_bisweep_trial{2}, 'result/signal.csv'); %% descend 
