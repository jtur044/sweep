function ret_info = video_sweep_analyzer (input_dir, info)

% VIDEO_SWEEP_ANALYZER 
%
%   ret_info = video_sweep_analyzer (input_dir, info)
%
% where 
%       input_dir is the directory of the okndetector result  
...
%   result/
%       okn/
%           clip-1-right_down-sweep_disks/      
%           clip-3-right_up-sweep_disks/
%
...
for k = 1:N 
    ...
    %if (check_required_dir (each_bisweep_dir))
        ...
        ret_info.descending = analyze_unidirectional_sweep (descendTbl, "descend", info);
        ret_info,ascending = analyze_unidirectional_sweep (descendTbl, "descend", info);
